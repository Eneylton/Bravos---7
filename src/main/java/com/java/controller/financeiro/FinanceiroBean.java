...
public class FinanceiroBean implements Serializable {
	...
	private List<Financeiro> listarRecibo;
	private List<Financeiro> listarRelatorio;
	...
	private void carregarLista() throws SQLException {
		...
		listaFinanceiros = financeiroService.listarTodosUsuariosPorAutoEscola(idAuto);
		pesquisarFinanceiros = financeiroService.listarTodosUsuariosPorAutoEscola(idAuto);
	}
	...
	public void gerarRecibo() {
		...
		try {
			...
			listarRecibo = financeiroService.retornoIdRecibo(idRecibo);

			String caminho = Constantes.CAMINHO_FISICO_BASE_WEBAPP + "resources/relatorios/RelatorioID.jrxml";
			JasperReport relatorio = JasperCompileManager.compileReport(caminho);
			JRBeanCollectionDataSource dados = new JRBeanCollectionDataSource(listarRecibo, false);
	...
	public void gerarRelatorio() {
		...
		try {
			...
			listaFinanceiros = financeiroService.listarTodosUsuariosPorAutoEscola2(idAuto);

			String caminho = Constantes.CAMINHO_FISICO_BASE_WEBAPP + "resources/relatorios/TransacaoFinaceira.jrxml";
			JasperReport relatorio = JasperCompileManager.compileReport(caminho);
			JRBeanCollectionDataSource dados = new JRBeanCollectionDataSource(listaFinanceiros, false);
			JasperPrint print = JasperFillManager.fillReport(relatorio, null, dados);
	...
	private void carregarValor() throws SQLException {
		...
		receita = painelFinanceiroService.getTotalAtualizado(idAuto);
	...
	public List<Financeiro> getListarRecibo() {
		return listarRecibo;
	}

	public void setListarRecibo(List<Financeiro> listarRecibo) {
		this.listarRecibo = listarRecibo;
	}

	public List<Financeiro> getListarRelatorio() {
		return listarRelatorio;
	}

	public void setListarRelatorio(List<Financeiro> listarRelatorio) {
